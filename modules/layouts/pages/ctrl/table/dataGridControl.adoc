...
Обязательный параметр, на основании которого происходит поиск плагина для формирования таблицы. Например, плагин `EDIMessagesLog`, входящий в комплект поставки модуля "{em}", подробнее см. в разделе "xref:dev@edi::module-structure.adoc#grid-plugins[Плагины для грида]".
