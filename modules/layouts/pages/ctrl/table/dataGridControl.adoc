...
Обязательный параметр, на основании которого происходит поиск плагина для формирования таблицы. Например, плагин `EDIMessagesLog`, входящий в комплект поставки модуля "{em}", подробнее см. в разделе "xref:6.1@edi::module-structure.adoc#grid-plugins[Плагины для грида]".

NOTE: Создание плагина подробнее описано в xref:programmer:server/data-grid-control.adoc[руководстве разработчика].
