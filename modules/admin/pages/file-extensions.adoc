...
. Измените значение параметра `AcceptableFileExtensions`. Если параметр отсутствует, создайте его самостоятельно. Значение содержит строку регулярного выражения. Новые расширения файлов нужно добавлять во вторые скобки, через символ `|`.
+
