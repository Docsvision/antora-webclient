:link: https://github.com/Docsvision/WebClient-Samples/tree/master/ServerExtensions/ShiftTasksEndDate
...
. Скопируйте каталог `SamplesOutput\Extensions\ShiftTasksEndDateServerExtension` в каталог `Путь к сайту Web-клиента\Extensions`.
. Скопируйте файл `SamplesOutput\Extensions\ru\ShiftTasksEndDateServerExtension.resources.dll` в каталог "`Путь к сайту Web-клиента\Extensions\ru`".
. Скопируйте файл `SamplesOutput\Extensions\uk\ShiftTasksEndDateServerExtension.resources.dll` в каталог `Путь к сайту Web-клиента\Extensions\uk`.
. Скопируйте каталог`SamplesOutput\Content\Modules\ShiftTasksEndDateWebExtension` в каталог `Путь к сайту Web-клиента\Content\Modules`.
