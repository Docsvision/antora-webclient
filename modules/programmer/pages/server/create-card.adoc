:link: https://github.com/Docsvision/WebClient-Samples/tree/master/ServerExtensions/CreateCard
:name: CreateCard
...
. Скопируйте каталог `SamplesOutput\Content\Modules\CreateCardWebExtension` в каталог `Путь к сайту Web-клиента\Content\Modules`.
. Скопируйте каталог `SamplesOutput\Extensions\CreateCardServerExtension` в каталог `Путь к сайту Web-клиента\Extensions`.
...
<.> Создаёт новый экземпляр `LayoutWebClientExtension`. `serviceProvider` -- сервис-провайдер.
