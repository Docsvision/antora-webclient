...
. Скопируйте каталог `SamplesOutput\Site\Extensions\CustomConditionTypesInterfaces` в каталог `Путь к сайту Web-клиента\Extensions`.
. Скопируйте каталог `SamplesOutput\Site\Extensions\CustomConditionTypesLayoutExtension` в каталог `Путь к сайту Web-клиента\Extensions`.
