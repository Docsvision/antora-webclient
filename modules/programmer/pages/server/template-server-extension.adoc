...
[WARNING]
====
Расширения можно устанавливать в папки menu:Site[Extensions] и menu:Site[bin]. В ходе разработки расширения можно удобнее устанавливать в папку menu:Site[bin], т.к. это избавляет от необходимости перезапускать *{wcs}* при установке сборок.

В ресурсной поставке рекомендуется устанавливать сборки в папку menu:Site[Extensions], чтобы проще было видеть, какие расширения установлены в {wc}е. Однако этот способ требует перезапуска *{wcs}* при установке или обновлении сборки.
====
+
. Перезапустите *{wcs}*.
