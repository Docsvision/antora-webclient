:link: https://github.com/Docsvision/WebClient-Samples/tree/master/ClientScripts/TypeScript/GridContextMenuItem
:name: GridContextMenuItem

= Расширить контекстное меню

Грид позволяет расширить функциональность контекстного меню при помощи расширений. Данный раздел описывает расширение, представляющее собой клиентский скрипт, который позволяет добавить новый пункт контекстного меню. Новый пункт контекстного меню позволяет открыть карточку в модальном окне или скрыть существующий элемент.
