...
. Сохраните изменения и перезапустите *{wcs}* (для загрузки скриптов).
...
NOTE: Чтобы получить другие примеры, включая пример получения списка файлов для отображения столбца _Правки_, обратитесь к исходному коду элемента управления `_Ход согласования_` в каталоге установки {wc}а: `C:\Program Files (x86)Docsvision\WebClient\Site\Content\App\Approval\Controls\AgreementHistory`.
