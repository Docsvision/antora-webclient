...
* https://www.visualstudio.com/[Visual Studio] {vscode-v}.
// * Docsvision WebClient SDK (запрашивается через портал ТП).

== Описание файлов проекта

* В корне репозитория расположено решение для Visual Studiofootnote:[Рекомендуется использовать Visual Studio Code {vscode-v}.] (`Samples.sln`) и ключ для подписания сборок примеров (`StrongNameKey.snk`).
