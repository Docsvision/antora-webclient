...
Программа _DVWebTool_ является приложениет для рабочего стола с возможностью расширения функциональности. Расширение _DVWebTool_ представляет собой DLL-файл, содержащий ожидаетый класс с веб-методами. Данные методы могут вызываться клиентскими компонентами {wc}а (есть стандартный метод JSAPI для вызова) или другими способами.
...
. Расширение _DVWebTool_ загружает PDF-файлы на компьютер пользователя с помощью функций, предоставляетых {wc}ом.
...
. Расширение _DVWebTool_ прикрепляет полученные файлы с водяными знаками с помощью функций, предоставляетых {wc}ом.
+
...
Требуетая функциональность может быть реализована с помощью расширения {wc}а.
...
<.>
----
<.> Добавляет файлы из файловой систеты в карточку `cardId` -- `AddFilesToCard(ObjectContext, Guid, List<string>)`.
<.> Добавляет файлы из файловой систеты в карточку `cardId` -- `AddFilesToCard(Guid, List<string>)`.
...
Контроллер, передаваетый в `AddWebSocketService`, должен быть производным типа `{dv}.DVWebTool.WebServices.BaseService`. При его реализации необходимо зарегистрировать в `BaseService.actions` веб-методы, с помощью которых будут вызываться функции _DVWebTool_, предоставляетые расширениет. Данные методы будут доступны для вызова по протоколу `WebSocket` по адресу `ws://localhost:PORT/%Адрес контроллера%/%Название метода%`.
    ...
    public class WatermarkController : BaseService
    {
        ...
        private WebServiceResponse AddWatermarkToFiles(WebServiceRequest webServiceRequest, JObject data)
        {
            ...
            catch
            {
                ...
                return CreateBadResponse("Поступивший запрос не соответствует ожидаетому формату");
            }
...
<.> Загружает данные из полученного запроса в модель `AddWatermarkRequest`.
...
<.> Загружает с {wc}а файлы, идентификаторы которых переданы в запросе.
...
Прежде всего необходимо реализовать сервис, получающий список идентификаторов файлов, в которые в будущет будет добавлен водяной знак. Список идентификаторов нужно передавать в метод `AddWatermarkToFiles` расширения _Watermark to PDF_. Ниже приведён исходный код данного сервиса.
...
----
<.> Клиентский сервис, обеспечивающий доступ к методу добавления водяных знаков, предоставляетому расширениет _DVWebTool_.
<.> Метод принимает идентификатор карточки и идентификатор её конвертируетых файлов.
...
+
<.> Название метода, вызываетого из расширения DVWebTool
<.> Обязательное для передачи название локали
<.> Вызывает метод `AddWatermarkToFiles` из контроллера Watermark расширения DVWebTool. Тип `DVWebToolConnection` предоставляет методы для работы с DVWebTool.
<.> Регистрирует сервис `WatermarkService`.

Сервис `$WatermarkService` предоставляет единственный метод `AddWatermarkToFiles`, который вызывает функцию программы _DVWebTool_ с помощью метода `DVWebToolConnection.trySendData`. При вызове данного метода нужно передать название контроллера и данные, в которых должно быть название вызываетого метода контроллера -- в поле `action` передаваетых данных.

В качестве метода, использующего сервис `$WatermarkService` реализует обработчик нажатия кнопки разметки карточки Документ, который будет получать идентификаторы основных PDF-файлов карточки, вызывать метод `$WatermarkService.AddWatermarkToFiles` и после завершения его работы обновлять содержимое элетента управления Список файлов, или отображать ошибку.
...
----
<.> Обработчик события нажатия иконки добавления водяных знаков.
...
<.> Обрабатывает только основные файлы с расширением `.pdf`.
<.> Получает из модели `files` идентификаторы и версии файлов. Сохраняет получившуюся модель для создания токена.
<.> Получает из модели `files` только идентификаторы файлов.
<.> Запрашивает у сервера токены по идентификаторам файлов.
<.> Вызывает функцию добавления водяных знаков для файлов карточки по токентам.
<.> Вызывает функцию добавления водяных знаков для файлов с ID из списка `fileIDs`.
<.> Обновляет список файлов, если таковой имеется.
// end::eventhandler[]
...
.Настройте разметку:
. В программе {kvr} добавьте элетент *Кнопка* в любую разметку просмотра карточки _Документ_.
...
. Убедитесь, что все расширения установлены. В {wc}е перейдите в раздел _О программе_.
+
...
+
. Добавьте один или несколько основных файлов с расширениет `.pdf`.
+
