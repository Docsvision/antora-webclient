...
<.> Получает идентификатора файла из последней версии.
...
. Запустите программу `{wcd}/Tools/Mage/mageui.exe`.
. Обновите манифест программы:
.. Нажмите menu:File[Open] и выберите файл `{wcd-win}/Tools/DVWebTool/ApplicationFiles/Docsvision.DVWebTool.exe.manifest`.
...
.. Перейдите в раздел _Application Reference_, нажмите кнопку *Select Manifest* и выберите файл `{wcd-win}/Tools/DVWebTool/Application FilesDocsvision.DVWebTool.exe.manifest`. В поле Version будет указана версия, полученная при обновлении файла манифеста.
