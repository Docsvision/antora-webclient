...
<.> Возвращает файл из карточки файла с версиями с `fileCardID`.
...
<.> Получение идентификатора текущей карточки и список файлов из элемента `FileList`
<.> Обработка только основных файлов с расширением `.pdf`
<.> Получение из модели `files` только идентификаторы файлов
<.> Получение реализованный сервис для работы с водяными знаками
<.> Вызов функцию добавления водяных знаков для файлов с идентификаторами из списка `fileIDs`
<.> Обновление список файлов, если есть. Возможно, уже открыта другая карточка, но в данном случае это не существенно.
...
. Скопируйте папку `SamplesOutput/Content/Tools/DVWebTool/Application Files/` в `{wcd}/Content/Tools/DVWebTool/`.
...
.. Нажмите menu:File[Open] и выберите файл `{wcd}/Content/Tools/DVWebTool/Application     FilesDocsvision.DVWebTool.exe.manifest`.
.. Перейдите в раздел _Name_ и в поле `Version` измените номер сборки. Например, `6.1.505+7cea24b736` до `6.1.505+7cea24b737`.
+
...
.. Нажмите menu:File[Open] и выберите файл `{wcd}/Content/Tools/DVWebToolDocsvision.DVWebTool.application`.
.. Перейдите в раздел _Name_ и в поле `Version` измените номер сборки. Например, `6.1.505+7cea24b736` до `6.1.505+7cea24b737`.
+
WARNING: Не изменяйте мажорную и минорную версии, и версию исправления.
+
.. Перейдите в раздел _Update option_ и в поле `Version` введите номер версии, который был получен в разделе Name. Например, `6.1.505+7cea24b736`.
